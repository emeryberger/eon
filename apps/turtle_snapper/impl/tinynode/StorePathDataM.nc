...
void clearCache()
{
	...
	task void prepPath()
	{
		...
		__writebuf = node_out->chunkarr.chunks[0].data;
		 ...
		 __writebuf[3] = BTYPE_RTPATH;
		 __writebuf[4] = 0;
		 __writebuf[5] = 0;
		 call BitVec.set16((uint16_t*)(__writebuf+4), TOS_LOCAL_ADDRESS, TRUE);
		//data
		 
		memcpy(__writebuf+6, &pathcache[cacheidx].pathid, 2);
		memcpy(__writebuf+8, &pathcache[cacheidx].count, 2);
		memcpy(__writebuf+10, &pathcache[cacheidx].cost, 4);
		__writebuf[14] = pathcache[cacheidx].prob;
		__writebuf[15] = pathcache[cacheidx].srcprob;
		//16 bytes
		
		pathcache[cacheidx].valid = FALSE;
		node_out->chunkarr.chunks[0].length = 16; //better be less than 28
